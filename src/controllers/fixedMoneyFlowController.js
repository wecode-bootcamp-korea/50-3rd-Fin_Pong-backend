...
const getFixedMoneyFlowsByCondition = async (req, res) => {
  try {
    ...
    if (!year && (month || date)) { // 연도가 없고 월, 날짜 조건이 있는 경우  => 연도를 입력해 주세요
    ...
    else if (!year && !month && !date) { // 연도, 월, 날짜의 조건이 없는 경우 => 해당 가족의 고정 수입/지출내역을 모두 찾습니다.
      ...
      const fixedMoneyFlows = await fixedMoneyFlowService.getFixedMoneyFlowsByYearMonth(userId, year, month); // 해당 연, 월의 고정 수입/지출 내역을 찾습니다.
      return res.status(200).json({message: 'GET_SUCCESS', flows: fixedMoneyFlows});
    }
    else if (year && !month && date) { // 월 조건만 없고, 연, 날짜 조건만 있는 경우 (ex.2023년의 매달 1일에 무엇을 쓰고 벌었는 지 알려줘)
      const fixedMoneyFlows = await fixedMoneyFlowService.getFixedMoneyFlowsByYearDate(userId, year, date); // 해당 연도의 해당 날짜의 고정 수입/지출 내역들을 찾습니다.
      return res.status(200).json({message: 'GET_SUCCESS', flows: fixedMoneyFlows});
    }
...
const updateFixedMoneyFlows = async (req, res) => {
  try {
    ...
    const updateTargetFixedFlowIds = await fixedMoneyFlowService.getFlowIdsByGroupId(groupId);
    await fixedMoneyFlowService.updateFixedMoneyFlows(updateTargetFixedFlowIds, amount, type, category, memo);
